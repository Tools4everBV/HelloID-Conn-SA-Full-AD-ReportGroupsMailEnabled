...
Invoke-HelloIDDatasource -DatasourceName "AD-group-generate-table-report-mail-enabled" -DatasourceType "4" -DatasourceInput $tmpInput -DatasourcePsScript $tmpPsScript -DatasourceModel $tmpModel -returnObject ([Ref]$dataSourceGuid_0) 
...
Invoke-HelloIDDynamicForm -FormName "AD - Report - Groups with email enabled" -FormSchema $tmpSchema  -returnObject ([Ref]$dynamicFormGuid) 
<# END: Dynamic Form #>
...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD - Report - Groups with email enabled" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-info-circle" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
...
'@ 
	...
	Invoke-HelloIDAutomationTask -TaskName "AD-export-report-groups-mail-enabled" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD - Report - Groups with email enabled' already exists. Nothing to do with the Delegated Form task..." 
}
