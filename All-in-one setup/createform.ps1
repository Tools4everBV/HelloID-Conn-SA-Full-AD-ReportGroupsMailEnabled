...
try {
    ...
    if([string]::IsNullOrEmpty($response.automationVariableGuid)) {
        ...
        $body = @{
            name = "$variableName";
            value = '[{ "OU": "OU=HelloIDCreated,OU=Groups,OU=Enyoi,DC=enyoi-media,DC=local"}]';
            secret = "false";
            ItemType = 0;
        }
...
$taskName = "AD-group-generate-table-report-mail-enabled"
$taskGetAdGroupsGuid = ""
...
$dataSourceName = "AD-group-generate-table-report-mail-enabled"
...
try {
    ...
    if([string]::IsNullOrEmpty($response.dataSourceGUID)) {
        ...
        $body = @{
            name = "$dataSourceName";
            type = "3";
            model = @(@{key = "CanonicalName"; type = 0}, @{key = "CN"; type = 0}, @{key = "Description"; type = 0}, @{key = "Name"; type = 0});
...
$formName = "AD - Report - Groups with email enabled"
$formGuid = ""
...
try
{
        ...
        [
  {
    ...
    "body": "The following report will show local AD groups that have an email address. Please wait while the data is loading...",
...
$delegatedFormName = "AD - Report - Groups with email enabled"
$delegatedFormGuid = ""
...
$taskActionName = "AD-export-report-groups-mail-enabled"
$taskActionGuid = ""
